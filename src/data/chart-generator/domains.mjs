...
export function pickFlavor(rng) {
    ...
    return { year, region: reg.r, currency: reg.cur, industry, suffix, period };
}
...
export function buildDocPrompt(domain, { sectionsWanted, cols, rows, wantTitle, wantGroups, complex, dense, extreme, huge, panels, textFocus, complexCells, matrix, matrixMarker, spine, form, flavor }) {
    ...
    if (panels) {
        return [
            ...
            domain.instruction,
            "",
            `Produce EXACTLY ${panels} sections. Each section is a SELF-CONTAINED mini-report: a short heading, at most ONE short sentence of lead text, and ONE small table (about ${cols} columns and ${rows} rows). Keep conclusion/intro empty or very short. The ${panels} tables should cover related-but-distinct facets so the panels read like ${panels} little documents.`,
