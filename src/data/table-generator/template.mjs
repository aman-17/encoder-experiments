...
export function varyTemplate(t, rng) {
    ...
    if (v.body.fontFamily && rng.bool(0.7)) {
        ...
        // leak; draw order matches pre-flag code exactly (flag-off is unchanged).
...
// ---- hard visual styles (--hard-table-mix) ---------------------------------

// Border styles whose cell boundaries are not fully drawn — the styles layout
// detectors miss most (borderless, rule-only, header-only tables).
const SPARSE_BORDERS = ["none", "horizontal", "header-only", "top-bottom", "thick-header"];

// Force a visually-hard table style onto a template (mutates it): sparse or
// absent borders, often hairline width / low-contrast color, sometimes no
// zebra striping — the whitespace-only table look.
export function forceHardTableStyle(template, rng) {
    const tb = template.table;
    tb.border = rng.pick(SPARSE_BORDERS);
    if (rng.bool(0.5)) {
        tb.borderColor = hsl(0, 0, rng.int(72, 88)); // light gray, low contrast
    }
    if (rng.bool(0.4)) {
        tb.borderWidthPx = 0.5;
    }
    if (rng.bool(0.5)) {
        tb.zebra = false; // remove the row-stripe crutch too
    }
    if (tb.headerStyle === "band" && rng.bool(0.5)) {
        tb.headerStyle = "plain";
        tb.headerBg = "transparent";
    }
    template.name = `${template.name || "template"}~hard`;
    return template;
}

// Visual difficulty of a template's table styling, complementing the
// structural table_difficulty flag from tableModel: hard = essentially no
// visible cell grid (borderless, or sparse + hairline/low-contrast lines).
export function tableVisualDifficulty(template) {
    const tb = (template && template.table) || {};
    const sparse = SPARSE_BORDERS.includes(tb.border) || tb.border === "vertical";
    const lowContrast = luminance(tb.borderColor) >= 0.7;
    const hairline = (tb.borderWidthPx || 1) <= 0.5;
    if (tb.border === "none" || (sparse && (lowContrast || hairline))) {
        return "hard";
    }
    if (sparse || lowContrast) {
        return "medium";
    }
    return "easy";
}
...
export function templateToCss(t) {
  ...
  `;
    ...
    /* plain: headerless label-value / code listings (hard-miss shapes) — always
       borderless and content-sized regardless of the template's table style. */
    table.gen.plain, table.gen.plain td, table.gen.plain th { border: none; background: none; }
    table.gen.plain { width: auto; margin: 6px 0 10px; }
    table.gen.plain td { padding: 1.5px 26px 1.5px 0; }
    table.gen.plain td:last-child { padding-right: 0; }
