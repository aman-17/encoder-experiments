...
export function generateTableModel(rng, opts = {}) {
    // structureBoost (--hard-table-mix): skew toward the structures that make
    // detection hard — wider tables, grouped column headers, rowspan labels.
    const boost = Boolean(opts.structureBoost);
    const ncols = rng.weighted(boost
        ? [[3, 1], [4, 4], [5, 4], [6, 4], [7, 3], [8, 3], [10, 2], [12, 1]]
        : [[2, 1], [3, 3], [4, 4], [5, 4], [6, 3], [7, 2], [8, 2], [10, 1], [12, 1]]);
    const nrows = rng.weighted(boost
        ? [[6, 4], [8, 4], [10, 3], [14, 3], [20, 2], [28, 1]]
    ...
    const hasGroups = ncols >= 4 && rng.bool(boost ? 0.6 : 0.28);
    if (hasGroups) {
    ...
    const hasRowspan = !hasGroups && ncols >= 3 && nrows >= 6 && rng.bool(boost ? 0.55 : 0.22);
    if (hasRowspan) {
...
// ---- hard-miss shapes (complexity-100 miss mining, Jul 2026) ---------------
// The styles real detectors miss are exactly the ones this generator never
// produced: headerless borderless label–value / code listings embedded in
// prose (patent IPC blocks, References-Cited columns, spec lines, SEC cover
// micro-tables) and stacked sub-tables with repeated headers. DocLayNet's
// convention labels all of these Table, so we generate them as tables.

const IPC_LETTERS = "ABCFGH";
function ipcCode(rng) {
    const l1 = IPC_LETTERS[rng.int(0, IPC_LETTERS.length - 1)];
    return `${l1}${String(rng.int(1, 12)).padStart(2, "0")}${"BCDFGKLMNP"[rng.int(0, 9)]} ${rng.int(1, 99)}/${String(rng.int(0, 999)).padStart(2, "0")}`;
}
const SURNAMES = ["Kaneko", "Harley", "Brzozowy", "Matsumura", "Vanleenhove", "Ishikawa", "Palaniappan", "Smith", "Abe", "Hatakeyama", "Cao", "Nguyen", "Weber", "Rossi", "Larsen"];
const SPEC_LABELS = ["shaft", "lobe", "pump sealing", "wear plates", "body material", "spring material", "sealing parts", "flap disk ring", "seat", "connection type", "control pilot", "max. operating pressure", "diaphragm material", "casing", "impeller", "drive rating"];
const SPEC_VALUES = ["min. GG-25 cast iron", "C45 carbon steel or chrome-nickel-molybdenum steel", "helical shape, cast iron covered with NBR (minimum 3 wings)", "mechanical seal", "abrasion resistant steel", "min. GG-40 ductile iron", "min. 1.4401 (AISI 316) stainless steel", "1.4301 (AISI 304) stainless steel", "neoprene rubber", "flange connection DIN 2633", "pressure-reducing pilot, pressure-fixing pilot", "16 bars", "replaceable Buna-N material", "hot dip galvanized", "gooseneck shape with DIN 2633"];
const SYMBOL_TERMS = ["thin layer", "photoresist underlayer", "exposed region", "non-exposed region", "photoresist pattern", "organic layer pattern", "substrate", "etch mask", "spacer film", "hard mask", "contact plug", "gate stack"];

const td = (text, align = "left") => ({ tag: "td", text, colspan: 1, rowspan: 1, align, role: "body" });

// Headerless 2–3-column label–value listing, 1–6 rows, borderless (plain),
// colon- or gap-separated, meant to sit inline in prose with no title.
export function generateLabelValueModel(rng) {
    const variant = rng.weighted([["spec", 3], ["symbols", 2], ["sec_cover", 2]]);
    const rows = [];
    let ncols = 2;
    if (variant === "spec") {
        const n = rng.int(1, 6);
        const sep = rng.pick([":", ":", ""]);
        for (let r = 0; r < n; r++) {
            rows.push([td(`${rng.pick(SPEC_LABELS)}${sep}`), td(rng.pick(SPEC_VALUES))]);
        }
    } else if (variant === "symbols") {
        const n = rng.int(2, 6);
        ncols = rng.bool(0.5) ? 4 : 2;
        let num = rng.int(100, 118);
        for (let r = 0; r < n; r++) {
            const cells = [td(`${num}:`, "right"), td(rng.pick(SYMBOL_TERMS))];
            if (ncols === 4) { num += 2; cells.push(td(`${num}:`, "right"), td(rng.pick(SYMBOL_TERMS))); }
            num += 2;
            rows.push(cells);
        }
    } else { // SEC cover micro-table: security | symbol | exchange
        ncols = 3;
        const n = rng.int(1, 3);
        const classes = ["Common Stock, $0.01 par value", "Class A Common Stock", "Class B Common Stock", "Depositary Shares", "5.250% Senior Notes due 2031"];
        const exch = rng.pick(["New York Stock Exchange", "NASDAQ Global Select Market"]);
        for (let r = 0; r < n; r++) {
            rows.push([td(rng.pick(classes)), td(Array.from({ length: rng.int(2, 4) }, () => "ABCDEFGHKLMNPRSTVW"[rng.int(0, 17)]).join(""), "center"), td(exch)]);
        }
    }
    return {
        ncols, nrows: rows.length, rows, columns: [], plain: true,
        flags: { table_difficulty: "hard", layout_style: `label_value_${variant}`, headerless: true },
        meta: { hasTitle: false, hasGroups: false, hasRowspan: false, headerless: true, labelValue: true },
    };
}

// Patent-style headerless code listing: IPC classification columns or
// References-Cited citation rows. 4–12 rows, borderless, wide-gap columns.
export function generateCodeListingModel(rng) {
    const variant = rng.weighted([["ipc", 3], ["citations", 3]]);
    const rows = [];
    let ncols;
    if (variant === "ipc") {
        ncols = 2;
        const n = rng.int(4, 8);
        const year = rng.pick(["(2006.01)", "(2013.01)", "(2019.01)"]);
        for (let r = 0; r < n; r++) {
            rows.push([td(`${ipcCode(rng)}`), td(year, "right")]);
        }
    } else {
        ncols = 3;
        const n = rng.int(5, 12);
        for (let r = 0; r < n; r++) {
            const y = rng.int(1994, 2013);
            rows.push([
                td(`${y}/${String(rng.int(1, 999999)).padStart(7, "0")} ${rng.pick(["A1", "A1", "B1", "B2"])}`),
                td(`${rng.int(1, 12)}/${y}`, "right"),
                td(`${rng.pick(SURNAMES)}${rng.bool(0.7) ? " et al." : ""}`),
            ]);
        }
    }
    return {
        ncols, nrows: rows.length, rows, columns: [], plain: true,
        flags: { table_difficulty: "hard", layout_style: `code_listing_${variant}`, headerless: true },
        meta: { hasTitle: false, hasGroups: false, hasRowspan: false, headerless: true, codeListing: true },
    };
}

// Split a standard table into a stacked pair: both halves keep the header
// rows (the repeated-band-header geometry that bait-merges detectors). GT
// stays two separate tables.
export function splitStackedPair(model, rng) {
    const split = model.rows.findIndex((row) => row.some((c) => c.tag === "td"));
    if (split < 0) { return null; }
    const head = model.rows.slice(0, split).filter((row) => !(row.length === 1 && row[0].role === "title"));
    const body = model.rows.slice(split);
    if (body.length < 4 || !head.length) { return null; }
    const cut = rng.int(2, body.length - 2);
    const mk = (rowsBody) => ({
        ...model,
        rows: [...head, ...rowsBody], nrows: rowsBody.length,
        flags: { ...model.flags, layout_style: "stacked_pair" },
        meta: { ...model.meta, hasTitle: false, stackedPair: true },
    });
    return [mk(body.slice(0, cut)), mk(body.slice(cut))];
}
